...
for(const f of ['card-art.js','tutorial.js','tutorial-model.js','index.html','app.js','ads.js','ads-provider.js','legal-content.js','legal-ui.js','privacy.html','data.html','support.html','accessibility.html','terms.html','licenses.html','third-party-notices.txt','deal-intro.js','swap-effects.js','deal-rules.png','styles.css','config.js','store-config.js','vendor/supabase.js','vendor/native.js','fonts/dm-sans.woff2','fonts/manrope.woff2'])assert.equal(hash('public/'+f),hash('ios/App/App/public/'+f),'iOS assets out of sync: '+f);
...
for(const file of ['public/card-art.js','public/tutorial.js','public/tutorial-model.js','public/app.js','native.js','scripts/check-ios.mjs'])assert.equal(spawnSync(process.execPath,['--check',file]).status,0);
...
if(process.argv.includes('--release')){
 ...
 for(const key of ['supportEmail','developerName','publisherLocation','appleTeamId'])if(!settings[key])blockers.push('Set '+key+' in app-store.json');
