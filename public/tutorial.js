...
export function setupTutorial({isInGame=()=>false}={}){
 ...
 $('tutorialCoach').replaceChildren(node('small',`Lesson ${state.lesson+1} of ${lessons.length} · Step ${state.step+1} of ${lessons[state.lesson].steps.length}`),node('h3',step.title),node('p',step.instruction));if(state.lesson===0&&!state.review)$('tutorialCoach').append(node('small','(Replacing a card is optional. You can discard your drawn card instead, even if it has no special power.)','tutorial-optional'));if([3,4,5,6].includes(state.lesson))$('tutorialCoach').append(node('small','(In a real game, this power is optional — you can skip it.)','tutorial-optional'));
