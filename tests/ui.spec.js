...
test('mobile table, private peek, turn lock, settings persistence, reconnect and scoring',async({browser},testInfo)=>{
 ...
 await expect(a.locator('#hand .face')).toHaveCount(2);await expect(b.locator('#hand .face')).toHaveCount(2);await a.getByRole('button',{name:'I’m ready',exact:true}).click();await b.getByRole('button',{name:'I’m ready',exact:true}).click();
 ...
 await a.getByRole('button',{name:'Draw from deck'}).click();await expect(a.locator('#drawnSlot')).toBeVisible();await expect(a.locator('#drawnCard')).toHaveAttribute('aria-label',/^You drew /);await a.reload();await expect(a.locator('#drawnSlot')).toBeVisible();await expect(a.locator('#drawnCard')).toHaveAttribute('aria-label',/^You drew /);await a.getByRole('button',{name:'Your card 1',exact:true}).click();await a.getByRole('button',{name:'Replace selected card',exact:true}).click();
